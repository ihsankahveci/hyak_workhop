...
## aka. DELTA METHOD
fit = glm(Salary ~ -1 + Country + OpenSource + Remote + YearsCodedJob,
          data = stackoverflow)
...
N = 100
data = stackoverflow
formula = Salary ~ -1 + Country + OpenSource + Remote + YearsCodedJob
coef_df = list()
...
## last line orders our table alphabetically for easier comparison.
alpha = 0.05
bootsrap_CI = coef_df |> 
  group_by(term) |> 
  summarise_all(list(
